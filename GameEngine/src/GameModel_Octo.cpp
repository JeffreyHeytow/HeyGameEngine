#include <sb6.h>
#include <assert.h>
#include "GameModel_Octo.h"
#include "File.h"

struct VertCoordinate_Octo
{
	float x;
	float y;
	float z;
...
#define TRILIST_TRIANGLE_COUNT (sizeof(tri_List) / sizeof(tri_index_Octo))

#define CUBE_DATA_VERT_COUNT (sizeof(cubePositions)/sizeof(VertCoordinate_Octo))


GameModel_Octo::GameModel_Octo(const char* const thisFileName)
	: GameModel()
{
	this->generateVAO(thisFileName);
}

GameModel_Octo::~GameModel_Octo()
{
	//remove anything dynamic here!
}

void GameModel_Octo::generateVAO(const char* const vaoFileName)
{
	FileHandle file;
	FileError error;
	...
	*/
	...
	tri_index_Octo* indexPtr = (tri_index_Octo*)(intPtr + 2);
	VertCoordinate_Octo* ptr = (VertCoordinate_Octo*)(dataBuffer + (sizeof(int) * 2) + (sizeOfTriIndex * numberOfTriIndices) + (sizeof(int) * 2));
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(VertCoordinate_Octo) * this->vertCount, ptr, GL_STATIC_DRAW);

	//vertex data in location 0:
	void *vertOffset = (void *)((unsigned int)&ptr[0].x - (unsigned int)ptr);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(VertCoordinate_Octo), vertOffset);
	glEnableVertexAttribArray(0);

	//texture data in location 1:
	void *textOffset = (void*)((unsigned int)&ptr[0].u - (unsigned int)ptr);
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(VertCoordinate_Octo), textOffset);
	glEnableVertexAttribArray(1);

	//norm data in location 2:
	void *normOffset = (void *)((unsigned int)&ptr[0].lx - (unsigned int)ptr);
	glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(VertCoordinate_Octo), normOffset);
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(tri_index_Octo) * this->trisCount, indexPtr, GL_STATIC_DRAW);
}
